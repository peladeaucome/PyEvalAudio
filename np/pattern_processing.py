...
class Mixin:
    def excitationPatternProcessing(
        ...
        EsTilde_R = self.AR_filter(
            X = EsTilde_R,
            alpha=self.patternProcessingAlpha
        )

        EsTilde_T = self.AR_filter(
            X = EsTilde_T,
            alpha=self.patternProcessingAlpha
        )
        ...
        momentaryCorrection = np.square(momentaryCorrection).reshape(numChannels, 1, numFrames)
        ...
        patternCorrectionFactor_ref = self.AR_filter(
            X = correlationFreqSmooth_ref,
            alpha = self.patternProcessingAlpha
        )
        patternCorrectionFactor_test = self.AR_filter(
            X = correlationFreqSmooth_test,
            alpha = self.patternProcessingAlpha
        )
    ...
    def modulationPatternProcessing(
        ...
        Ebar_R:npt.ArrayLike = self.AR_filter(
            X =Es_R,
            alpha=np.power(self.timeToFreqAlpha, .3)
        )
        ...
        Dbar_R = self.AR_filter(
            X = np.abs(np.power(Es_R,.3)-np.power(Es_R_prev,.3)), alpha=self.timeToFreqAlpha
        )*self.Fss_fft
        Dbar_T = self.AR_filter(
            X = np.abs(np.power(Es_T,.3)-np.power(Es_T_prev,.3)), alpha=self.timeToFreqAlpha
        )*self.Fss_fft

        # M_T, M_R
        M_T:npt.ArrayLike = Dbar_T/(1+Ebar_T/.3)
        M_R:npt.ArrayLike = Dbar_R/(1+Ebar_R/.3)
    ...
    def loudnessCalculation(
        ...
        commonFactor=c*np.power(self.Et/(self.s*E0), .23).reshape(1, self.numBarkBands, 1)

        # N_R, N_T
        N_T = np.power(
            1 - self.s + self.s*EsTilde_T/self.Et
            ,.23
        )-1
        N_T = N_T*commonFactor

        N_R = np.power(
            1 - self.s + self.s*EsTilde_R/self.Et
            ,.23
        )-1
        N_R = N_R*commonFactor

        Ntot_T:npt.ArrayLike = np.sum(np.maximum(N_T, 0), axis=1)*24/self.numBarkBands
        Ntot_R:npt.ArrayLike = np.sum(np.maximum(N_R, 0), axis=1)*24/self.numBarkBands
